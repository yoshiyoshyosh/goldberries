...
foreach ($lines as $line) {
  $i++;
  if ($is_debug)
    echo "Checking line #$i: $line\n";

  // if ($i > 20) {
  //   echo "Stopping after 10 lines\n";
  //   break;
  // }
  ...
  if ($is_debug)
    echo "Comparing timestamps: " . json_encode($timestamp) . " vs " . json_encode($most_recent_timestamp) . "\n";
  ...
  if ($most_recent_timestamp !== null && $timestamp <= $most_recent_timestamp) {
    // echo "Skipping old line #$i -> $timestamp\n";
    if ($is_debug)
      echo "Skipping old line #$i\n";
    continue;
  }
  ...
  if ($skip_page) {
    if ($is_debug)
      echo "Skipping page request: $page\n";
    continue;
  }
  ...
  if ($skip) {
    if ($is_debug)
      echo "Skipping bot request #$i: $skip_agent\n";
    continue;
  }
  ...
  if ($result === false) {
    if ($is_debug)
      echo "Failed to insert line #$i: " . pg_last_error($DB) . "\n";
  } else {
    if ($is_debug)
      echo "Inserted line #$i\n";
  }
}
